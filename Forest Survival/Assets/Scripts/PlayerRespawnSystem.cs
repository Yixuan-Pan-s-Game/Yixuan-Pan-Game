...
// player respawn system script that owns this feature's runtime behavior.
public static class PlayerRespawnSystem
{
    // Handles the respawn workflow.
    public static bool Respawn(PlayerHealth playerHealth)
    {
        ...
        PlayerToolController inventory = player.GetComponent<PlayerToolController>();
        if (inventory != null)
        {
            inventory.ApplyDeathInventoryPenalty();
        }
    ...
    // Finds, loads, or caches the references needed for resolve respawn anchor.
    ...
    // Sets state, selection, or placement data for snap respawn position to ground.
    ...
    // Finds, loads, or caches the references needed for find exact in scene.
    ...
    // Finds, loads, or caches the references needed for find by name contains.
